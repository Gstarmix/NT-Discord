...
7. **Liens internes** : format `**[texte](/path)**` (gras + lien, URL relative). Uniquement des chemins réels du site. Si un chemin est incertain, ne pas inventer : renvoyer vers `**[nostar.fr](https://nostar.fr)**`.
   Chemins connus : `/raids`, `/raids/<slug>`, `/sp/simulator`, `/sp/list`, `/xp/simulator`, `/main-quests`, `/progress` (`/progress/a1` à `/a10`), `/forum`, `/dmg`, `/dmg-pve`, `/stuffs`, `/faq/overview`, `/community/family|fame|dignity|contribution|popularity|cuarry` (guides communautaires), `/class`, `/shortcut`, `/pc` (ajoutés au prompt le 2026-07-10).
